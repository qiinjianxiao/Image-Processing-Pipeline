function green = green_interpolation_RI(mosaic, mask, pattern, sigma)
...
if strcmp(pattern,'grbg')||strcmp(pattern,'GRBG')
    maskGr(1:2:size(rawq,1), 1:2:size(rawq,2)) = 1;
    maskGb(2:2:size(rawq,1), 2:2:size(rawq,2)) = 1;
elseif strcmp(pattern,'rggb')||strcmp(pattern,'RGGB')
...
v = 0;
eps = 0;
tentativeRh = guidedfilterR(Guidegh, mosaic(:,:,1), mask(:,:,1), h, v, eps);
tentativeGrh = guidedfilterR(Guiderh, mosaic(:,:,2).*maskGr, maskGr, h, v, eps);
tentativeGbh = guidedfilterR(Guidebh, mosaic(:,:,2).*maskGb, maskGb, h, v, eps);
tentativeBh = guidedfilterR(Guidegh, mosaic(:,:,3), mask(:,:,3), h, v, eps);
tentativeRv = guidedfilterR(Guidegv, mosaic(:,:,1), mask(:,:,1), v, h, eps);
tentativeGrv = guidedfilterR(Guiderv, mosaic(:,:,2).*maskGb, maskGb, v, h, eps);
tentativeGbv = guidedfilterR(Guidebv, mosaic(:,:,2).*maskGr, maskGr, v, h, eps);
tentativeBv = guidedfilterR(Guidegv, mosaic(:,:,3), mask(:,:,3), v, h, eps);
